...
| `docs/figure1-conceptual-framework.html` → `.png` | `node render-figure.mjs` | Figure 1 |
| `docs/figure2-domain-scores.png` | `node render-figure.mjs docs/sample-paper.html … ".figure .chart"` | Figure 2 |

`.gitignore` blanket-ignores `*.png`; both figures are exempted so the paper's
own assets stay tracked while screenshots do not.
